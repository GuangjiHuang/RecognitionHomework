...
def CaridDetect(car_pic):
	...
	for card_index,card_img in enumerate(card_imgs):
		...
		# 如果出现了空矩阵会报错，所以使用循环来跳过去
		if card_img.shape[0] == 0:
			continue
...
if __name__ == '__main__':
	...
	for img_i in os.listdir(path):
		roi, label,color = CaridDetect(path+"/"+img_i)
		print(f"正在显示{img_i}图片的车牌")
		cv2.imshow("segmentation_1.jpg",roi)
		cv2.waitKey(2000)