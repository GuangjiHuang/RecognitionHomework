...
def get_feature(img, split_num, rate, reverse=0, show=0, cut_or_not=1):
    """
    输入图像的灰度图和分块以及比率判断，返回的就是一个图像的特征
    :param img: 输入单通道的图像数组
    :param split_num: 分成多少块，其中特征的个数就是分块值的平方
    :param rate: 黑字像素占的比例
    :param reverse: 是否需要将和1反转
    ...
    if cut_or_not == 1:
        ...
        res_row = res_col = 240
        img_cut = cv.resize(img, (res_row, res_col), interpolation=cv.INTER_LINEAR)
        # show the image
    # 看下输入数据的直方图
    # plt.hist(img)
    # plt.show()
    ...
    # 分块后的每个小块的行列数
    k = int(img.shape[0]/row) if cut_or_not else int(240/row)
    # 复制一份数据，用来画网格线
    ...
    if cut_or_not:
        ...
        img_feature_show = 255*np.ones((240, 240),dtype="uint8")
    for i in range(row):
        for j in range(column):
            # 两个矩阵相乘
