...
class PaintBoard(QWidget):
    ...
    def __InitData(self):
        ...
        # 本质上就是在QPixmap中画图，这里的__painter应该就是一个工具或者画笔
        self.__painter = QPainter()#新建绘图工具
        # 这里包括初始化了画笔的粗细，画笔的颜色，画笔颜色的列表
    ...
    def __InitView(self):
        #设置界面的尺寸为__size
        self.__size_view = QSize(340, 340)
        self.setFixedSize(self.__size_view) #证明这里的__size虽然是在上面的函数中出现，因为是放进了实例对象创建中，所以可以被这个函数访问
        self.setStyleSheet("PaintBoard{background:rgb(0,0,255,255);}")
    def Clear(self):
        #清空画板
    ...
    def GetContentAsQImage(self):
        ...
        print("显示画板的图片", image, "类型为：", type(image))
        return image
    ...
    def mouseMoveEvent(self, mouseEvent):
        ...
        if self.EraserMode == False:
            ...
            #橡皮擦模式下画笔为纯白色，粗细为30，橡皮檫本质上就是一个白色的画笔而已
            self.__painter.setPen(QPen(Qt.white,30))
